...
class GazeImporter2(object):
    ...
    def set_valid_paths(self, path):
        self.valid_paths = [(path, path + 'output/')]
    ...
    def process_data(self, inpath, outpath):
        ...
        fpn = '/'.join(rootpath.split('/')[:-3]) + '/world.intrinsics'
    ...
    def images_to_video(self, inpath, fourcc, fps, width, height):
        ...
        n = int(
            sorted([x[2] for x in os.walk('/'.join(inpath.split('/')[:-1]))][0])[-1].split('.jpg')[0].split('frame')[
                -1])
        frames = np.arange(0, n, 1)
        for f in frames:
            print('..saving video {:.2f} %'.format(100 * f / frames[-1]), end='\r')
            im = cv2.imread(inpath % f)
            writer.write(im)
        writer.release()
...
def main(inpath, outpath=None):
    gazeImporter = GazeImporter2(inpath, outpath)
    gazeImporter.run()
